...
function formatTopCategories(array $categories, float $monthOutflow) {
    if (empty($categories)) {
        return ["  No expense data for this month."];
    }

    $lines = [];
    foreach ($categories as $i => $cat) {
        $pct      = $monthOutflow > 0 ? round(($cat['total'] / $monthOutflow) * 100) : 0;
        $lines[]  = ($i + 1) . ". " . $cat['name'] . ": "
            . number_format($cat['total'], 2) . " TND (" . $pct . "%)";
    }
    return $lines;
}
...
function formatRecentTransactions(array $transactions) {
    ...
    foreach ($transactions as $tx) {
        $catLabel = $tx['category_name'] ? "[" . $tx['category_name'] . "]" : "[Uncategorized]";
        $desc     = $tx['description'] ? " — " . $tx['description'] : "";
        $sign     = $tx['type'] === 'inflow' ? "+" : "-";
        $lines[]  = "  " . $tx['transaction_date'] . " " . $catLabel . " "
            . $sign . number_format($tx['amount'], 2) . " TND" . $desc;
    }
    return $lines;
}
